...
1. **Self-update**: `git pull` to pick up code changes
2. Resolves CellarTracker credentials from 1Password via `op read`
3. Fetches in parallel: inventory + notes + food tags + menu calendar
4. Parses inventory + menu in parallel → `data/inventory.json`, `data/menu.json`
5. **Generates plan** (rules-based) → `data/plan.json` (staging)
6. **Generates tasting notes** (Claude CLI, augmented with CT notes/food tags) → `data/plan.json`
7. Compares inventory vs plan → `data/report.json`
8. Generates pairing suggestions → `data/pairing_suggestions.json`
9. **Publishes atomically**: copies plan + pairings + report to `site/`

The site always has a complete plan with notes. The new plan is only published after all steps succeed.
...
## Step 3 — Plan Generation (Automated)

The plan is now generated automatically by `scripts/generate_plan.py` every pipeline run. Manual regeneration is no longer needed.

- Plan is staged to `data/plan.json`, not written directly to `site/`
- Notes are generated by Claude CLI and added to the staged plan
- Only after all steps complete is the plan published to `site/plan.json`
- **Do not modify `site/index.html`** — it is a presentation shell
- All wine metadata comes from `data/inventory.json` (CellarTracker is the system of record)
- The LLM generates tasting notes only — scheduling is rules-based
